...
Status: Active (`M0` + `M1` complete, `M2` + `M3` complete, `M4` wiring complete, `M7`-`M10` hardening complete, `M5` complete, `M6` in progress)  
...
|---|---|---|
| `M0` | Complete | Baseline audit + ownership map + tracking checklist are formalized in `governance/1.0.0_M0_BASELINE.md`. |
| `M1` | Complete | Frozen-surface contract test closure is complete: all six M1 test slices are implemented and full milestone gates are green (`./scripts/semantic_suite.sh`, `./scripts/authority_parity.sh`). |
...
| `M5` | Complete | Final documentation sweep is complete: contract-facing docs were reconciled, generated reference docs refreshed, and stale canonical-example references were rechecked/cleared for active workflows. |
| `M6` | In progress | Release-candidate gates are green; final acceptance status flip in `spec/1.0.0_STABILITY_CONTRACT.md` remains tied to the `v1.0.0` release tag. |

`M0` gate evidence captured on 2026-02-28:

1. Baseline contract-clause gap matrix published: `governance/1.0.0_M0_BASELINE.md` (`FS-*`, `RC-*`, `LC-*`, `CG-*`, `NG-*` clause mapping).
2. Ownership map published with subsystem accountability (`compiler`, `runtime`, `tooling`, `docs`, `release`).
3. Tracking checklist published with milestone evidence links and explicit deferral note for open milestone PR links (`M6`).
4. Blocking unknowns review completed (`None`; all remaining work mapped to owners + milestones).
...
`M1` gate evidence captured on 2026-02-28:

1. `./scripts/semantic_suite.sh` (`PASS`; includes parser fixtures, sema goldens, config/decode boundary behavior, and AST/native/AOT authority gates)
2. `./scripts/authority_parity.sh` (`PASS`; includes canonicalization, AST/native parity, AOT parity lock matrix, and panic taxonomy gates)
3. `governance/1.0.0_M1_TEST_SLICES.md` closure checklist (`M1-PR1` through `M1-PR6`) fully checked and linked to gate subsets
...
`M5` gate evidence captured on 2026-02-28:

1. `./scripts/generate_guide_docs.sh` (`PASS`; regenerated `docs/site/specs/reference.md` and companion generated docs)
2. Final-sweep consistency check across:
   - `README.md`
   - `guides/fuse.md`
   - `spec/fls.md`
   - `spec/runtime.md`
   - `governance/scope.md`
3. Canonical-example trace check (`PASS`; no stale `notes-api`/`auth-api` references in active docs/scripts/benchmarks)

`M6` gate evidence captured on 2026-02-28:

1. `./scripts/semantic_suite.sh` (`PASS`)
2. `./scripts/authority_parity.sh` (`PASS`; includes AST/native/AOT parity-lock + panic taxonomy gates)
3. `./scripts/lsp_suite.sh` (`PASS`; via `./scripts/reliability_repeat.sh --iterations 2`)
4. `./scripts/reliability_repeat.sh --iterations 2` (`PASS`; includes repeated parity/LSP/use-case benchmark/AOT SLO gate path)
5. `./scripts/check_aot_perf_slo.sh` (`PASS`; via `./scripts/reliability_repeat.sh --iterations 2` and `./scripts/release_smoke.sh`)
6. `./scripts/packaging_verifier_regression.sh` (`PASS`; via `./scripts/release_smoke.sh`)
7. `./scripts/release_smoke.sh` (`PASS`)
8. Remaining acceptance action:
   - `spec/1.0.0_STABILITY_CONTRACT.md` status flip `Proposed` -> `Accepted` at `v1.0.0` tag cut.
9. `v0.7.0` release-prep metadata sync (`PASS`):
   - crate/tool versions bumped to `0.7.0` (`crates/fuse`, `crates/fusec`, `crates/fuse-rt`, `tools/vscode`)
   - release-facing changelogs/docs aligned for the `0.7.0` line (`CHANGELOG.md`, `tools/vscode/CHANGELOG.md`,
     `README.md`, `ops/AOT_RELEASE_CONTRACT.md`, `ops/AOT_ROLLBACK_PLAYBOOK.md`, `ops/RELEASE.md`)
