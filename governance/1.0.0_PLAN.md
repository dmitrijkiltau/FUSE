...
Status: Active (`M0` complete, `M1` in progress, `M2` + `M3` complete, `M4` wiring complete, `M7`-`M10` hardening complete, `M5` complete, `M6` in progress)  
...
| `M1` | In progress | Contract-test coverage was expanded during `v0.6.0` work (capabilities, typed error domains, structured concurrency, transaction semantics, strict-architecture checks, and new request/response HTTP primitives); remaining frozen-surface coverage is now enumerated into concrete PR slices in `governance/1.0.0_M1_TEST_SLICES.md`. |
...
3. Tracking checklist published with milestone evidence links and explicit deferral note for open milestone PR links (`M1`, `M6`).
