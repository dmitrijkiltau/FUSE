...
| `M7` | In progress | AOT runtime contract hardening started: explicit runtime contract text + startup determinism tests are being landed (startup order, metadata determinism, config precedence, no hidden startup execution path). |
