...
1. **Bump version** across all locations:
   ```sh
   ./scripts/bump_version.sh 0.9.5
   ```
   Dry-run to verify what will change without writing:
   ```sh
   ./scripts/bump_version.sh --dry-run 0.9.5
   ```
2. **Update `CHANGELOG.md`:**
   - Move relevant items from `Unreleased` into the new version section (e.g. `## [0.9.5] - 2026-03-07`).
3. **Run preflight** — verifies version consistency, changelog, guide freshness, authority parity, smoke, AOT SLO, and benchmark regression gates:
   ...
   ```sh
   ./scripts/release_preflight.sh --skip-bench 0.9.5
   ```
   ...
   ```sh
   ./scripts/release_preflight.sh --workspace-publish-checks 0.9.5
   ```
   Force a cold release cache run by clearing all repo `.fuse-cache` directories first:
   ```sh
   ./scripts/release_preflight.sh --clear-fuse-cache 0.9.5
   ```
