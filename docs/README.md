...
The Docker image is runtime-only. It does not build Rust or compile FUSE in-container.
Only a prebuilt docs AOT binary plus static site assets/SVGs are copied into the image.
...
Prepare build inputs first:

```bash
./scripts/generate_guide_docs.sh
./scripts/fuse build --manifest-path docs --release
FUSE_AOT_BUILD_INFO=1 docs/.fuse/build/program.aot
```
