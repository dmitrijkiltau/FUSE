...
# The default image tag is v0.9.6. Override with FUSE_VERSION to test a different patch tag:
#   FUSE_VERSION=v0.9.6 docker compose ... up --build
...
```bash
export FUSE_VERSION=v0.9.6
docker compose -f examples/reference-service/docker-compose.yml up --build
```

This overrides the `${FUSE_VERSION:-v0.9.6}` default in `docker-compose.yml` and is passed through
